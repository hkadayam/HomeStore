...
pub trait IndexOps: Send + Sync {
    ...
    /// Register a new snapshot. Bumps `GLOBAL_SEQ` and returns the unique `ts`.
    /// Returns `Err(InvalidOperation)` on plain (non-MVCC) indices.
    fn register_snapshot(&self) -> Result<u64>;

    /// Deregister a snapshot identified by `ts`. No-op on plain indices.
...
pub(crate) fn err_not_mvcc(method: &str) -> HomeDbError {
    HomeDbError::InvalidOperation(format!(
        "{method} requires an MVCC table (set mvcc_enabled = true in TableSpec)"
    ))
}
