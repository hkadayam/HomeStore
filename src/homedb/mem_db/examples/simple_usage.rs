...
//! - Initializing the MemoryDB singleton
...
use mem_db::{init_mem_homedb, mem_homedb, shutdown_mem_homedb, TableSpec};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=== MemDB Simple Usage Example ===\n");
    
    // Initialize the MemoryDB singleton (4 reactor threads)
    println!("1. Initializing MemoryDB...");
    init_mem_homedb(4)?;
    let db = mem_homedb();
