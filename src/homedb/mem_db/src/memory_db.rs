...
use std::sync::Arc;
use dashmap::DashMap;
#[allow(unused_imports)] // RangeIterator used in return types when sync_mode or async_mode is enabled
use crate::{
    table::Table,
...
impl MemoryDB {
    ...
    pub fn new() -> Result<Self> {
        ...
        {
            return Ok(Self { tables: DashMap::new() });
        }
    ...
    pub fn drop_table(&self, name: &str) -> Result<()> {
        self.tables.remove(name).ok_or_else(|| MemDbError::TableNotFound(name.to_string()))?;
    ...
    pub fn list_tables(&self) -> Vec<String> { self.tables.iter().map(|entry| entry.key().clone()).collect() }
