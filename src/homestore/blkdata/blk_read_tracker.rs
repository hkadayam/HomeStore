...
mod tests {
    ...
    async fn test_wait_on_no_pending_reads() {
        ...
        // Should complete immediately since no pending reads
        let result = tracker.wait_on(&blkid).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_wait_on_with_pending_reads() {
        ...
        // Spawn a task to wait on it
        let tracker_clone = Arc::new(tracker);
        let tracker_ref = Arc::clone(&tracker_clone);
        let wait_task = iomgr::spawn(async move {
            tracker_ref.wait_on(&blkid).await
        });

        // Give the wait task a moment to register
        iomgr::sleep(std::time::Duration::from_millis(10)).await;

        // Remove the read - should unblock the waiter
        tracker_clone.remove(&blkid);
    ...
    async fn test_spanning_aligned_ranges() {
        ...
        let tracker_ref = Arc::clone(&tracker_arc);
        
        // Wait on it - should wait for all ranges to be free
        let wait_task = iomgr::spawn(async move {
            tracker_ref.wait_on(&blkid).await
        });
