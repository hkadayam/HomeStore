...
public:
    ...
    static Async< shared< RawBlkStream > > create(uint64_t stream_id, MetaClient& meta_client,
    ...
    static Async< shared< RawBlkStream > > load(uint64_t stream_id, MetaClient& meta_client,
                                                const std::string& dev_name, const shared< VirtualDev >& vdev,
    ...
    /// frees via vdev.
    Async< void > invalidate_blk(CP* cp, const BlkId& bid);

    /// Bulk invalidate — sequentially invalidates each BlkId via invalidate_blk.
    Async< void > invalidate_blks(CP* cp, BlkIds const& bids);
    ...
    Async< void > write(const BlkId& bid, const sisl::IoBuf& buf, bool buffered = false);
    ...
    Async< void > write_multi(BlkIds const& bids, sisl::IoBuf const& buf, bool buffered = false);
    ...
    Async< void > writev_multi(BlkIds const& bids, sisl::SgList const& sg, bool buffered = false);
    ...
    /// else {}.
    Async< std::error_code > read_multi(BlkIds const& bids, sisl::IoBuf& buf);
    ...
    Async< void > buffered_write_flush();
