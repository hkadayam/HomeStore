...
use super::btree_req::{BtreeRemoveRequest, BtreeRemoveAnyRequest, BtreeRangeRemoveRequest, BtreeKeyRange, RemoveFilterFn};
...
impl<'a, K: BtreeKey + 'static, V: BtreeValue + 'static> RemoveContext<K, V> for RemoveOneContext<'a, K, V> {
    async fn execute_on_leaf(&mut self, btree: &Btree<K, V>, leaf: &mut Node) 
        -> Result<u32, BtreeError> {
        let (found, idx) = leaf.find::<K, V>(self.key);
        if !found { return Ok(0); }
        
        let value = leaf.get_nth_value::<K, V>(idx, true);
        leaf.remove::<K, V>(idx)?;
...
impl<'a, K: BtreeKey + 'static, V: BtreeValue + 'static> RemoveContext<K, V> for RemoveRangeContext<'a, K, V> {
    async fn execute_on_leaf(&mut self, btree: &Btree<K, V>, leaf: &mut Node) -> Result<u32, BtreeError> {
        ...
        let num_removed = leaf.multi_remove::<K, V>(self.range, u32::MAX, self.filter_fn)?;
        if num_removed > 0 {
            btree.storage.write_node(leaf).await?;
        }
        Ok(num_removed)
    ...
    V: BtreeValue + 'static,
{
    ...
    pub(in super::super) async fn remove_one_internal(&self, req: BtreeRemoveRequest<'_, K>) 
    ...
    pub(in super::super) async fn remove_any_internal(&self, req: BtreeRemoveAnyRequest<K>) 
    ...
    pub(in super::super) async fn remove_range_internal<'a>(&self, req: BtreeRangeRemoveRequest<'a, K, V>) 
                               -> Result<u32, BtreeError> {
    ...
    where R: RemoveContext<K, V> {
        ...
        while curr_idx <= end_idx {
            ...
            if self.is_merge_needed(&child) {
                let node_end_idx = if my_node.has_valid_edge() {
                    my_node.total_entries()
                } else {
                    my_node.total_entries() - 1
                };
    ...
    pub(in super::super) async fn merge_child_nodes(&self, parent_node: &Node, leftmost_node: &mut Node, 
        start_idx: u32, end_idx: u32) -> Result<bool, BtreeError> {
        ...
        while idx <= end_idx {
            ...
            if idx == end_idx {
                let copied = new_ref.append_copy_in_upto_size::<K, V>(old_ref, &mut src_cursor, 
                                             self.config.ideal_fill_size, /*copy_only_if_fits=*/true);
                if !copied {
                ...
                new_ref.append_copy_in_upto_size::<K, V>(old_ref, &mut src_cursor, 
                                             self.config.ideal_fill_size, /*copy_only_if_fits=*/false);
        ...
        for new_node_ref in new_nodes[1..].iter().rev() {
            ...
            if new_node_ref.total_entries() > 0 {
                let last_key = new_node_ref.get_last_key::<K, V>().ok_or(
                    BtreeError::Io(io::Error::new(io::ErrorKind::InvalidInput,
        ...
        if leftmost_node.total_entries() > 0 {
            let last_key = leftmost_node.get_last_key::<K, V>().ok_or(BtreeError::Io(io::Error::new(io::ErrorKind::InvalidInput,
