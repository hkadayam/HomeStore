...
FollyRpcClientFactory::create_client(std::string const& endpoint) {
    // Parse "host:port"
...
PeerOutboundSocket&
FollyRpcClientFactory::get_or_open_outbound(std::string const& host, uint16_t port) {
...
size_t FollyRpcClientFactory::pick_reactor_for_cold_path(std::string const& host,
                                                         uint16_t port) const {
    auto key = make_key(host, port);
