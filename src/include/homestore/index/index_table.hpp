...
public:
    ...
    void repair_node(IndexBufferPtr const& idx_buf) override {
        if (idx_buf->is_meta_buf()) {
            // We cannot repair the meta buf on its own, we need to repair the root node which modifies the meta_buf. It
            // is ok to ignore this call, because repair will be done from root before meta_buf is attempted to repair,
            // which would have updated the meta_buf already.
            return;
        }
        ...
        // Set the cp_id to current cp buf, since repair doesn't call get_writable_buf (unlike regular IO path), so we
        // need to set it here, so that other code path assumes correct cp
        ...
        if (idx_buf->m_up_buffer && idx_buf->m_up_buffer->is_meta_buf()) {
            // Our up buffer is a meta buffer, which means that we are the new root node, we need to update the meta_buf
            // with new root as well
...
protected:
    ...
    btree_status_t repair_links(BtreeNodePtr const& parent_node, void* cp_ctx) {
        ...
        LOGINFO("Repairing node={} with last_parent_key={}", parent_node->to_string(), last_parent_key.to_string());
        ...
        do {
            ...
            LOGINFO("Repairing node={} child_node={} child_last_key={}", parent_node->node_id(),
