...
protected:
    void SetUp() override {
        ...
        m_iodev = iomgr().spawn_and_block(ReactorTarget::reactor(0), DriveInterface::open_dev(g_dev_path, O_RDWR));
...
TEST_F(DriveTest, SingleReactorWriteReadVerify) {
    ...
    iomgr().spawn_and_block(ReactorTarget::reactor(0), [this, &drive, num_blocks]() -> Async< void > {
        ...
        iomgr().spawn_waitable_all_seq(
            [this, &drive, per_reactor, ios_per_reactor](size_t reactor_id) -> Async< uint32_t > {
                ...
                for (uint32_t i = 0; i < ios_per_reactor; ++i) {
                    ...
                    const uint64_t offset = region_start + (static_cast< uint64_t >(i) * kBlockSize) % per_reactor;
                    // round down to block boundary
                    const uint64_t aligned = (offset / kBlockSize) * kBlockSize;
                    if (aligned + kBlockSize > region_end)
                        continue;
