...
void LogDev::stop() {
    ...
    // Before we call stop, we need to do any pending device truncations
    truncate();
...
void LogDev::start_timer() {
    ...
    iomanager.run_on_wait(logstore_service().flush_thread(), [this]() {
        m_flush_timer_hdl =
            iomanager.schedule_fiber_timer(HS_DYNAMIC_CONFIG(logstore.flush_timer_frequency_us) * 1000,
                                           true /* recurring */, nullptr /* cookie */, [this](void*) { flush(); });
    });
}
...
int64_t LogDev::append_async(logstore_id_t store_id, logstore_seq_num_t seq_num, const sisl::io_blob& data,
    ...
    if (allow_inline_flush()) {
        auto const threshold_size = flush_data_threshold_size();
        auto const prev_size = m_pending_flush_size.fetch_add(data.size(), std::memory_order_relaxed);
        if (prev_size < threshold_size && ((prev_size + data.size()) >= threshold_size)) {
            if (m_flush_mtx.try_lock()) { do_flush(); }
        }
    }
    return idx;
}
...
LogGroup* LogDev::prepare_flush(int32_t estimated_records) {
    ...
    lg->finish(m_logdev_id, m_prev_crc);
...
bool LogDev::flush(int64_t threshold_size) {
    if (!iomanager.can_run_blocking_io()) {
        iomanager.run_on_forget(logstore_service().flush_thread(), [this, threshold_size]() { flush(threshold_size); });
        return false;
    }
...
void LogDev::on_flush_completion(LogGroup* lg) {
    ...
    for (auto idx = from_indx; idx <= upto_indx; ++idx) {
        ...
        HS_LOG_ASSERT(req->is_write, "Incorrect completion of IO, something wrong with request");
    ...
    HISTOGRAM_OBSERVE(logstore_service().m_metrics, logdev_post_flush_processing_latency, get_elapsed_time(done_time));
    free_log_group(lg);
...
uint64_t LogDev::truncate() {
    ...
    for (auto& [store_id, store] : m_id_logstore_map) {
        ...
        auto const [trunc_lsn, trunc_ldkey, tail_lsn] = store->truncate_info(lstore->truncated_upto());
        if (trunc_lsn == tail_lsn) {
            THIS_LOGDEV_LOG(DEBUG, "Store_id={} didn't have any writes since last truncation, skipping ", store_id);
            continue;
        }
        HS_DBG_ASSERT_GE(key.idx, m_last_truncate_idx, "Trying to truncate logid which is already truncated");
        ...
        if (trunc_ld_key.idx < min_safe_ld_key.idx) { min_safe_ld_key = trunc_ld_key.idx; }
    }
    ...
    HS_PERIODIC_LOG(INFO, logstore, "Truncating log_dev={} log_id={} vdev_offset={} truncated {} log records",
                    m_logdev_id, min_safe_ld_key.idx, min_safe_ld_key.dev_offset, num_records_to_truncate);

    // Truncate them in vdev
    m_vdev_jd->truncate(min_safe_ld_key.dev_offset);
    THIS_LOGDEV_LOG(DEBUG, "LogDev::truncation done upto log_id={} ", min_safe_ld_key.idx);

    // Update the start offset to be read upon restart
    m_last_truncate_idx = min_safe_ld_key.idx;
    m_logdev_meta.set_start_dev_offset(min_safe_ld_key.dev_offset, min_safe_ld_key.idx + 1, false /* persist_now */);
    ...
    m_logdev_meta.remove_rollback_record_upto(min_safe_ld_key.idx, false /* persist_now */);
...
void LogDev::on_log_store_found(logstore_id_t store_id, const logstore_superblk& sb) {
    ...
    LOGINFO("Found a logstore log_dev={} log_store={} with start lsn={}, Creating a new HomeLogStore instance",
            m_logdev_id, store_id, sb.m_first_lsn);
    logstore_info& info = it->second;
    info.log_store = std::make_shared< HomeLogStore >(shared_from_this(), store_id, info.append_mode, sb.m_first_lsn);
