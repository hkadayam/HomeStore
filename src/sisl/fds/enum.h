...
public:
    ...
    EnumSupportBase(const std::string& tokens_string) {
        ...
        while (current_pos < tokens_string.size()) {
            ...
            if ((delim != std::string::npos) && (tokens_string[delim] == '=')) {
                ...
                if (value_to_tokens_.size() != 0) ++last_value;
                current_pos = ((delim != std::string::npos) ? delim + 1 : delim);
            }
            value_to_tokens_[last_value] = token;
            token_to_value_[token] = last_value;
        }
    }
    ...
    [[nodiscard]] const std::string& get_name(const enum_type enum_value) const {
        ...
        const auto itr{value_to_tokens_.find(val)};
        return (itr == std::cend(value_to_tokens_)) ? unknown : itr->second;
    }

    [[nodiscard]] enum_type get_enum(const std::string& name) const {
        const auto itr{token_to_value_.find(name)};
        return (itr == std::cend(token_to_value_)) ? static_cast< enum_type >(0)
                                                   : static_cast< enum_type >(itr->second);
    }

    [[nodiscard]] size_t get_count() const { return value_to_tokens_.size(); }

private:
    std::unordered_map< underlying_type, std::string > value_to_tokens_;
    std::unordered_map< std::string, underlying_type > token_to_value_;
};
