...
SettingsFactoryRegistry::SettingsFactoryRegistry(const std::string& path,
        ...
        config_path_{path} {
    if (SISL_OPTIONS.count("config_path") != 0) { config_path_ = SISL_OPTIONS["config_path"].as< std::string >(); }
    ...
    for (const auto& cfg : cfgs) {
        ...
        const auto it{override_cfgs_.find(schema_name)};
        if (it != std::cend(override_cfgs_)) {
            it->second.merge_patch(j);
        } else {
            override_cfgs_.emplace(std::move(schema_name), std::move(j));
        }
    }
}
...
void SettingsFactoryRegistry::register_factory(const std::string& name, SettingsFactoryBase* f) {
    ...
    const auto config_file{fmt::format("{}/{}.json", config_path_, name)};
    {
        ...
        const auto it{override_cfgs_.find(name)};
        if (it != std::cend(override_cfgs_)) {
        ...
        f->load();
        factories_.insert(std::make_pair(name, f));
    }
}
